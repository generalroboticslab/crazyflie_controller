...
from _analysis.model_eval import evaluate_model
...
def create_replay_buffer_from_real_data(cfg):
    ...
    for each_folder in csv_full_folders:
        ...
        for each_file in csv_files:
            if each_file == "traj_1.csv": # only 1 traj at a time for model training
                file_path = os.path.join(each_folder, each_file)
                data = pd.read_csv(file_path)
                if "CF2" in cfg.overrides.env:
                    obs = data.iloc[:, 1:14].values
                    actions = data.iloc[:, [17, 14, 15, 16]].values # this order is important: (thrust (N), roll_rate, pitch_rate, yaw_rate (rad/s))
                    actions[:, 1:4] *= np.pi / 180.0  # convert to rad/s
                    obs = obs[:-1]
                    actions = actions[:-1]
                    next_obs = data.iloc[1:, 1:14].values
                else:
                    assert False, "This env is not supported yet"
...
if __name__ == "__main__":
    ...
    def run(cfg: omegaconf.DictConfig):
        ...
        """
        Fist create the replay buffer from real data.
        """
